...
class ConfigurationsController extends Controller
{
  ...
  public function index()
  {
    ...
    foreach ($configurations as $config_item) {
      ...
      return view('vendor.ticket.admin.configuration.index', compact('configurations', 'configurations_by_sections'));
  }
  ...
  {
      return view('vendor.ticket.admin.configuration.create');
  }
  ...
  public function store(Request $request)
  {
    ...
    return redirect()->action('Admin\ConfigurationsController@index');
  }
  ...
  public function edit($id)
  {
      ...
      return view('vendor.ticket.admin.configuration.edit', compact('configuration', 'should_serialize', 'default_serialized'));
  }
  ...
  public function update(Request $request, $id)
  {
      ...
      if ($request->serialize) {
          ...
          if (!Auth::attempt($request->only('password'), false, false)) {
              return back()->withErrors([trans('vendor.ticket.admin.config-edit-auth-failed')]);
          }
          if (false === eval('$value = serialize('.$value.');')) {
              return back()->withErrors([trans('vendor.ticket.admin.config-edit-eval-error')]);
          }
      }

      $configuration->update(['value' => $value, 'lang' => $request->lang]);

      Session::flash('configuration', trans('Setting has been modified', ['name' => $request->name]));
      \Cache::forget('settings'); // refresh cached settings
